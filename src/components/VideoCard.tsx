...
const VideoCard = forwardRef<VideoCardHandle, VideoCardProps>((props, ref) => {
  const {
    id,
    title = '',
    query = '',
  ...
  const actualSource =
    from === 'playrecord' && typeof id === 'string' && id.includes('+')
      ? id.split('+')[0]
      : source;
  const actualId =
    from === 'playrecord' && typeof id === 'string' && id.includes('+')
    ...
    if (origin === 'live' && actualSource && actualId) {
      ...
      const sourceParam =
        typeof actualSource === 'string'
          ? actualSource.replace('live_', '')
          : actualSource;
      const idParam =
        typeof actualId === 'string' ? actualId.replace('live_', '') : actualId;
      ...
      }${doubanIdParam}${actualSearchType ? `&stype=${actualSearchType}` : ''}${
      ...
      const url = `/play?id=${actualId}&title=${encodeURIComponent(
        actualTitle
      )}${actualYear ? `&year=${actualYear}` : ''}${doubanIdParam}${
        actualSearchType ? `&stype=${actualSearchType}` : ''
      }`;
