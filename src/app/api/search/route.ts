...
export async function GET(request: NextRequest) {
  ...
  const mergeResults = searchParams.get('merge') !== 'false';
      ...
      for (const result of flattenedResults) {
        ...
        if (uniqueResults.has(key)) {
          ...
          if (existingResult.episodes && existingResult.episodes_titles) {
            existingResult.episodes.forEach(
              (episode: string, index: number) => {
                if (existingResult.episodes_titles[index]) {
                  seenTitles.set(
                    episode,
                    existingResult.episodes_titles[index]
                  );
                }
              }
            );
          ...
          if (!existingResult.source_sites) {
            ...
            !existingResult.source_sites.includes(result.source_name)
          ) {
