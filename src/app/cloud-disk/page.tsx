  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(
        `/api/cloud-disk/search?kw=${encodeURIComponent(keyword)}`
      );
      ...
      const validatedData = {
        ...
        data: {
          ...
          merged_by_type: {
            baidu: Array.isArray(data.data?.merged_by_type?.baidu)
              ? data.data.merged_by_type.baidu
              : [],
            quark: Array.isArray(data.data?.merged_by_type?.quark)
              ? data.data.merged_by_type.quark
              : [],
          },
        },
      };
      ...
      // 如果百度网盘结果超过10个且夸克网盘结果少，自动折叠夸克网盘结果
        ...
        baiduResults.length > 10 &&
        quarkResults.length > 0 &&
        quarkResults.length < baiduResults.length
      ) {
          ...
          <div className='container mx-auto px-4 py-6'>
            ...
            {results && (
              <div className='space-y-6'>
                ...
                {/* 夸克网盘结果 - 优先展示 */}
                {results.data.merged_by_type.quark &&
                  results.data.merged_by_type.quark.length > 0 && (
                    <div>
                      ...
                      <div
                        className={`grid gap-4 ${
                          quarkCollapsed ? 'hidden' : ''
                        }`}
                      >
                        {results.data.merged_by_type.quark.map(
                          (item, index) => (
                            <div
                              key={index}
                ...
                {/* 百度网盘结果 */}
                {results.data.merged_by_type.baidu &&
                  results.data.merged_by_type.baidu.length > 0 && (
                    <div>
                      <div className='flex items-center justify-between mb-4'>
                        <h2
                          className='text-xl font-semibold text-gray-900 dark:text-white cursor-pointer hover:text-green-600 dark:hover:text-green-400 transition-colors flex items-center'
                      ...
                      <div
                        className={`grid gap-4 ${
                          baiduCollapsed ? 'hidden' : ''
                        }`}
                      >
                        {results.data.merged_by_type.baidu.map(
                          (item, index) => (
                            <div
                              key={index}
                              className='bg-white dark:bg-gray-800 rounded-lg p-4 shadow-sm border border-gray-200 dark:border-gray-700'
