...
export default function MyWatchingPage() {
  ...
  const loadPlayRecords = async (skipUpdateCheck = false) => {
    try {
      ...
      const [recordsObj] = await Promise.all([getAllPlayRecords()]);
  ...
  const checkForUpdates = async (
    records: (PlayRecord & { id: string })[]
  ): Promise<ExtendedPlayRecord[]> => {
    ...
    for (const record of records) {
      try {
        ...
        const detailResponse = await fetch(
          `/api/detail?source=${matchedResult.source}&id=${matchedResult.id}`
        );
        ...
        const latestEpisodes = detailData.episodes
        ...
        const shouldShowUpdate =
          hasUpdate &&
          (!userWatchedLatest || latestEpisodes > record.total_episodes);

        console.log(
          `检查更新 - ${record.title}: 原集数=${record.total_episodes}, 最新集数=${latestEpisodes}, 用户观看到第${record.index}集, 用户看到最新=${userWatchedLatest}, 有更新=${hasUpdate}, 应显示更新=${shouldShowUpdate}, 新增=${newEpisodes}集`
        );
  ...
  const handleConfirmClearStats = async () => {
    try {
      ...
      await Promise.all([clearUserStats(), clearAllPlayRecords()]);
              ...
              <div className='flex items-center justify-around'>
                <div className='text-center'>
                  <div className='text-2xl font-bold text-blue-600 dark:text-blue-400'>
                    {userStats
                      ? Math.floor(userStats.totalWatchTime / 3600)
                      : 0}{' '}
                    h
                  </div>
          ...
          <div className='hidden sm:block'>
            <div className='grid grid-cols-3 gap-4 mb-6'>
              <div className='glass-light rounded-xl p-4 text-center'>
                <div className='text-2xl font-bold text-blue-600 dark:text-blue-400'>
                  {userStats ? Math.floor(userStats.totalWatchTime / 3600) : 0}{' '}
                  h
                </div>
                <div className='text-sm text-gray-600 dark:text-gray-400'>
                  观看总时长
