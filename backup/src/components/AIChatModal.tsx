  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const errorMessage: Message = {
        ...
        type: 'ai',
        content:
          '抱歉，网络连接出现问题，请检查网络后重试。如果问题持续存在，请稍后再试。',
        timestamp: new Date(),
        recommendations: [],
        youtubeVideos: [],
      };
                      ...
                      <div className='w-16 h-20 bg-gray-200 dark:bg-gray-700 rounded flex-shrink-0 overflow-hidden'>
                        <img
                          ...
                          onError={(e) => {
                            ...
                            if (
                              parent &&
                              !parent.querySelector('.poster-placeholder')
                            ) {
                              const placeholder = document.createElement('div');
                              placeholder.className =
                                'poster-placeholder w-full h-full flex items-center justify-center text-gray-400 dark:text-gray-500 text-xs';
                    ...
                    <div className='mt-3 space-y-2 self-stretch'>
                      {message.recommendations.map((movie, index) => (
                        <div
                          ...
                          className='p-3 glass-light rounded-lg cursor-pointer hover:shadow-md transition-all'
                        >
                          <div className='flex items-start gap-3'>
                            {movie.poster && (
                              <img
                ...
                {message.youtubeVideos && message.youtubeVideos.length > 0 && (
                  <div className='mt-3 space-y-2 self-stretch'>
                    {message.youtubeVideos.map((video, index) => (
                      <div
                        key={index}
                        className='glass-light rounded-lg overflow-hidden'
                      >
