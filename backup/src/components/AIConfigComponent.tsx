...
export default function AIConfigComponent() {
  ...
  // 预设模型列表（用于判断是否为已知模型）
  const knownModels = aiModelOptions
  ...
  // 处理自定义模型输入
  const handleCustomModelInputChange = useCallback(
    (value: string) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const customModelValue =
            data.Config?.AIConfig?.customModel ||
            (isKnownModel ? '' : modelValue);
      ...
      <div className='space-y-4'>
        ...
        <div className='space-y-3'>
          ...
          <div className='relative' data-dropdown='ai-model'>
            ...
            >
              {aiModelOptions.find((option) => option.value === config.model)
                ?.label || '选择模型'}
            </button>
            ...
            {isModelDropdownOpen && (
              <div className='absolute z-50 w-full max-w-md mt-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg max-h-60 overflow-auto'>
                ...
                {[
                  ...
                  const groupOptions = aiModelOptions.filter(
